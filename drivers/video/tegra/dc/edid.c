...
int tegra_edid_read_block(struct tegra_edid *edid, int block, u8 *data)
{
	...
	do {
		...
		if (checksum != 0) {
			 ...
			 * a bad EDID checksum (specifically, some of the older TVs).
			 * These TVs have the modes, etc programmed in their EDID correctly,
			 * but just have a bad checksum. It then becomes hard to distinguish
			 * between an i2c failure vs bad EDID.
			 * To get around this, read the EDID multiple times. If the calculated
			 * checksum is the exact same multiple number of times, just print a
			 * warning and ignore.
			 */
...
"up on trying to change the i2c speed for EDID read\n");
	...
	/*
	 * Re-calculate the checksum since the standard EDID parser doesn't like the bad
	 * checksum
	 */
	if (last_checksum != 0) {
		u8 checksum = 0;
		...
		pr_warn("%s: remainder is %u for the last %d attempts. Assuming bad sink EDID and ignoring. New checksum is %u\n",
				__func__, last_checksum, TEGRA_EDID_MAX_RETRY, checksum);
	}
