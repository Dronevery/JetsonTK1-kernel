...
static int tegra_hdmi_get_extended_vic(const struct tegra_dc_mode *mode)
{
	...
	for (i = 1; i < HDMI_EXT_MODEDB_SIZE; i++) {
		 ...
		 * curr->refresh (or use the fb_mode_is_equal() which does this check).
		 * This is because of the lack of accuracy when going between
		 * fb_videomode -> tegra_dc_mode -> fb_videomode. For
		 * 1000/1001 modes, the calculated m->refresh is actually not accurate
		 * This is honestly a hack and does need to be fixed, but is a WAR
		 * for now for HDMI certification
		 */
